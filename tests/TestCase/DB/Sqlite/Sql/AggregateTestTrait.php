...
trait AggregateTestTrait
{
    ...
    {
        $this->assertSame(
            'SELECT SUM(DISTINCT CASE WHEN "test"."active" = 1 THEN "test"."total" END) OVER (PARTITION BY "test"."group_id" ORDER BY "test"."id" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW EXCLUDE CURRENT ROW) AS "total" FROM "test"',
            $this->db->select([
