...
trait WindowTestTrait
{
    ...
    public function testWindowExcludeCurrent(): void
    {
        $this->assertSame(
            'SELECT FIRST_VALUE("test"."value") OVER (ORDER BY "test"."id" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW EXCLUDE CURRENT ROW) AS "value" FROM "test"',
            $this->db->select([
    ...
    public function testWindowExcludeGroup(): void
    {
        $this->assertSame(
            'SELECT FIRST_VALUE("test"."value") OVER (ORDER BY "test"."id" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW EXCLUDE GROUP) AS "value" FROM "test"',
            $this->db->select([
                'value' => static fn(Query $query): WindowExpression => $query->func()
                    ->firstValue('test.value')
                    ->orderBy([
    ...
    public function testWindowExcludeTies(): void
    {
        $this->assertSame(
            'SELECT FIRST_VALUE("test"."value") OVER (ORDER BY "test"."id" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW EXCLUDE TIES) AS "value" FROM "test"',
            $this->db->select([
                'value' => static fn(Query $query): WindowExpression => $query->func()
                    ->firstValue('test.value')
                    ->orderBy([
    ...
    {
        $this->assertSame(
            'SELECT FIRST_VALUE("test"."value") OVER (ORDER BY "test"."id" ASC GROUPS BETWEEN 2 PRECEDING AND 1 FOLLOWING) AS "value" FROM "test"',
            $this->db->select([
                'value' => static fn(Query $query): WindowExpression => $query->func()
                    ->firstValue('test.value')
                    ->orderBy([
