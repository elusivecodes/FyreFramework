...
trait AggregateTestTrait
{
    ...
    {
        $this->assertSame(
            'SELECT COUNT(*) FILTER (WHERE "test"."active" = 1) AS "total" FROM "test"',
            $this->db->select([
                'total' => static fn(Query $query): AggregateExpression => $query->func()
                    ->count()
    ...
    {
        $this->assertSame(
            'SELECT SUM(DISTINCT "test"."total") FILTER (WHERE "test"."active" = 1) OVER (PARTITION BY "test"."group_id" ORDER BY "test"."id" ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS "total" FROM "test"',
            $this->db->select([
